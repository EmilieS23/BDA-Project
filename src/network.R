#Que faire de ce bout de code ? aucune idée
######################################################################
######################################################################
############## networking
...
df2 <- matrix_full_eco_clim_sat[,colnames(matrix_full_eco_clim_sat) %in% c("W_Ecosystm","temp","NDVI","precip")]

aggregated_df2 <- aggregate(. ~ W_Ecosystm, data = df2 , FUN = mean)

df2_hclust <- aggregated_df2[,colnames(aggregated_df2) %in% c("temp","NDVI","precip")]
