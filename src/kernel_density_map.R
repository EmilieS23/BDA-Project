...
matrix_fullfull_sp1 <- matrix_full_eco_elev_clim_sat[matrix_full_eco_elev_clim_sat$species =="Vulpes vulpes",]
matrix_fullfull_sp2 <- matrix_full_eco_elev_clim_sat[matrix_full_eco_elev_clim_sat$species =="Microtus arvalis",]
sf_use_s2(FALSE)

#################
############# kernel sp1 
sf_points1 <- data.frame(
   latitudesp = matrix_fullfull_sp1$latitudesp,
   longitudesp = matrix_fullfull_sp1$longitudesp
 ) |>
   st_as_sf(coords = c("longitudesp", "latitudesp"), crs = 4326)
...
sf_points2 <- data.frame(
   latitudesp = matrix_fullfull_sp2$latitude,
   longitudesp = matrix_fullfull_sp2$longitude
 ) |>
   st_as_sf(coords = c("longitudesp", "latitudesp"), crs = 4326)
...
# Create a function to generate the color palette
color_palette_sp2 <- colorRampPalette(c("white","darkred"))

# Define the number of colors in the palette
num_colors <- length(data_sp2$contlabel)  # Adjust as needed
# Generate the color palette
palette1 <- color_palette_sp2(num_colors)
